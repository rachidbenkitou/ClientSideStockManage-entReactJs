...
const Products = () => {
    ...
    const getPacks = async () => {
        setLoading(true);
        const response = await fetch("http://127.0.0.1:8000/api/packProduits");
        if (componentMounted) {
            const reponseData = await response.clone().json()

            if (reponseData.status === 200 && reponseData.packs && reponseData.packs.data && reponseData.packs.data.size !== undefined) {
                        ...
                        <div className={"col-md-3 mb-4"} key={pack.id}>
                            ...
                            <div className="card h-100 text-center p-4">
                                ...
                                <div className="card-body">
                                    ...
                                    <NavLink to={`/packages/${pack.id}`} className={"btn btn-outline-dark"}>Pack
                                        Details</NavLink>
                                </div>
                            </div>
